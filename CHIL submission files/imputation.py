...
class Imputation:
    ...
    def fit_impute(self, return_metrics=False):
        ...
        ADNI_DGP_NoNaNs_df.to_csv(
            'data/imputed_ADNI.csv', encoding='utf-8', index=False)
            ...
            for month in months:
                for col in data_cols:
                    ...
                    if(len(np.array(orig_data.loc[orig_data['VISCODE'] == month][col]))>1):
                        plt.subplot(1, 2, 1)
                        plt.hist(np.array(orig_data.loc[orig_data['VISCODE'] == month][col]), nbins)
                    ...
                    plt.subplot(1, 2, 2)
                    plt.hist(np.array(new_data.loc[new_data['VISCODE'] == month][col]), nbins)
                    plt.title(""+col+" imputed data month "+str(month))
                    plt.xlabel(col)
                    plt.grid()

                    plt.savefig('plots/imputed_'+col+'_'+str(month)+'.png', format='png', dpi=500)
                    plt.show()
        except:
            pass
            """
