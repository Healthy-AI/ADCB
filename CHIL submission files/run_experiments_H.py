...
def main(history=5):
    ...
    with open(config.plotting_pickle_prefix + 'history_' + str(history) + '.pickle', 'wb') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='run experiments history')
    parser.add_argument('-t', '--hhistory', type=int,
                        default=3, dest='hhistory')
    args = parser.parse_args()
    main(history=int(args.hhistory))
